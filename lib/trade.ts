...
export async function runTradeCycle(
  ...
  const { data: outbound } = await getSupabase()
    .from('artifacts')
    .select('*')
    .gt('trade_value', 3.0)
    .eq('source', 'local')
  ...
  if (outbound?.length) {
    await getSupabase().from('directory').upsert({
      agent_id: agentId,
      alias: agentAlias,
  ...
  const { data: peers } = await getSupabase()
    .from('peers')
    .select('*')
    .eq('status', 'connected')
    ...
    try {
      ...
      for (const pa of peerArtifacts) {
        ...
        if (!existing) {
          ...
          if (better) {
            await getSupabase().from('artifacts').update({
              output: pa.output,
              peer_rating: pa.trade_value ?? 0,
      ...
      await getSupabase().from('peers').update({ last_active: cycleTs }).eq('id', peer.id)
    } catch (e) {
  ...
  await getSupabase().from('trade_history').insert(event)
...
export async function connectToPeer(peer: {
  ...
  await getSupabase().from('peers').upsert({
