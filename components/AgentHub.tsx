...
import { supabase, Peer } from '@/lib/supabase'
...
export default function AgentHub({ settings }: { settings: Settings }) {
  ...
  const loadDirectory = async () => {
    setLoading(true)
    const { data } = await supabase.from('directory').select('*').order('avg_trade_value', { ascending: false })
    setDirectory((data ?? []).filter((d: DirectoryEntry) => d.agent_id !== settings.agentId))
    setLoading(false)
  }

  const loadPeers = async () => {
    const { data } = await supabase.from('peers').select('*').order('avg_trade_value', { ascending: false })
    setPeers(data ?? [])
  }
  ...
  const disconnect = async (peer: Peer) => {
    await supabase.from('peers').update({ status: 'blocked' }).eq('id', peer.id)
    await loadPeers()
  }
